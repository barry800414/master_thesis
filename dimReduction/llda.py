...
def load_corpus(filename):
    ...
    for line in f:
        ...
        if len(doc)>0:
            ...
            labels.append(label)
    f.close()
    return list(labelmap.keys()), corpus, labels
...
class LLDA:
    ...
    def set_corpus(self, labelset, corpus, labels):
        ...
        self.labelmap = dict(list(zip(labelset, list(range(len(labelset))))))
        self.K = len(self.labelmap)

        self.vocas = []
        self.vocas_id = dict()
        self.labels = numpy.array([self.complement_label(label) for label in labels])
        self.docs = [[self.term_to_id(term) for term in doc] for doc in corpus] # 2d list, convert term to id

        M = len(corpus) # doc num
        V = len(self.vocas) # volc size
        ...
        for m, doc, label in zip(list(range(M)), self.docs, self.labels):
            N_m = len(doc)
    ...
    def inference(self):
        V = len(self.vocas)
        for m, doc, label in zip(list(range(len(self.docs))), self.docs, self.labels):
